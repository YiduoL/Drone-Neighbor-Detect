# Copied from the official "dufomap" PyPI package's dufomap/utils/__init__.py (v1.1.1,
# pure Python/numpy, no compiled or platform-specific dependency). See
# detection/dufomap_custom/src/bind.cpp's header comment for why this local copy
# exists. One deviation from the original here: numpy.typing isn't available on numpy
# 1.17 (Jetson's system-site-packages numpy, needed for --system-site-packages to still
# see rosbag2_py/rclpy) -- these NDArray* names are only ever used as type-hint
# aliases (never runtime-checked), so falling back to plain np.ndarray when
# numpy.typing is missing changes nothing observable.
#
...
try:
    ...
    NDArrayBool = NDArrayFloat = NDArrayByte = NDArrayInt = np.ndarray
