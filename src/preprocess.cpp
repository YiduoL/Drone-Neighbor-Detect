...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg) {
    ...
    pl_full.clear();
    pl_nearfield.clear();
    const uint32_t plsize = std::min<uint32_t>(msg->point_num, msg->points.size());
    pl_corn.reserve(plsize);
    pl_surf.reserve(plsize);
    pl_full.resize(plsize);
    if (nearfield_enable) pl_nearfield.reserve(plsize);
    ...
    for (uint32_t i = 1; i < plsize; ++i) {
        ...
        // NOTE: point fields must be computed for every valid point (not just ones that
        // survive point_filter_num decimation below) because the near-field path needs
        // range for all of them, unconditionally of decimation phase.
        auto &point = pl_full[i];
        point.x = msg->points[i].x;
        point.y = msg->points[i].y;
        point.z = msg->points[i].z;
        point.intensity = msg->points[i].reflectivity;
        point.curvature = msg->points[i].offset_time / 1000000.0f;

        const auto &previous = msg->points[i - 1];
        const bool distinct = std::abs(point.x - previous.x) > 1e-7 ||
