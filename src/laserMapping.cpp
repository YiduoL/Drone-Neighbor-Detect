...
// Near-field deskew extension: parallel deque, kept index-aligned with lidar_buffer/time_buffer.
deque<PointCloudXYZI::Ptr> nearfield_buffer;
int nf_idx = 0;
...
struct PendingNearfieldPoint {
    double t;               // capture time (lidar_beg_time basis, matches pose_smoother nodes)
    Eigen::Vector3d p_body; // raw lidar-frame xyz (same convention as /nearfield/raw_body)
    float intensity;
    float range;
};
std::deque<PendingNearfieldPoint> os_deskew_pending;
PointCloudXYZI::Ptr refined_world_buf(new PointCloudXYZI());
PointCloudXYZI::Ptr refined_body_buf(new PointCloudXYZI());
std::vector<float> refined_ranges;
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    if (cut_frame) {
        ...
        for (int i = 0; i < ptr->size(); i++) {
            ...
            if (ptr->points[i].curvature / double(1000) + get_time_sec(msg->header.stamp) - time_div >
                ...
                *ptr_div_i = *ptr_div;
                ...
                // standard_pcl_cbk path has no near-field support (Livox-only feature);
                // push an empty cloud to keep nearfield_buffer index-aligned with lidar_buffer.
                nearfield_buffer.push_back(PointCloudXYZI::Ptr(new PointCloudXYZI()));
        ...
        nearfield_buffer.emplace_back(PointCloudXYZI::Ptr(new PointCloudXYZI()));
    }
...
void livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::SharedPtr msg) {
    ...
    PointCloudXYZI::Ptr nf_cloud(new PointCloudXYZI());
    if (p_pre->nearfield_enable) *nf_cloud = p_pre->pl_nearfield;
    if (!cloud->empty() || !nf_cloud->empty()) {
        lidar_buffer.emplace_back(cloud);
        time_buffer.emplace_back(stamp);
        nearfield_buffer.emplace_back(nf_cloud);
    }
    s_plot11[scan_count] = omp_get_wtime() - preprocess_start_time;
    sig_buffer.notify_all();
}
...
bool sync_packages(MeasureGroup &meas) {
    if (!imu_en) {
        if (!lidar_buffer.empty()) {
            ...
            meas.nearfield = nearfield_buffer.front();
            meas.lidar_beg_time = time_buffer.front();
            time_buffer.pop_front();
            lidar_buffer.pop_front();
            nearfield_buffer.pop_front();
...
// ---- Near-field deskew extension (see near-field design doc) ----
// Flush all pending near-field points (Measures.nearfield, time-sorted) whose timestamp
// is <= t_bound, transforming each through the CURRENT EKF state via the same
// pointBodyToWorld() the main /cloud_registered path uses. These points never go through
// update_iterated_dyn_share_modified() -- only a coordinate transform, no EKF update --
...
void flush_nearfield_upto(double t_bound) {
    ...
    while (nf_idx < (int) nf_pts.size()) {
        ...
        double t_nf = p_body.curvature / 1000.0 + Measures.lidar_beg_time;
        if (t_nf > t_bound) break;

        nearfield_raw_buf->points.push_back(p_body);

        PointType p_world;
        pointBodyToWorld(&p_body, &p_world);
        p_world.curvature = p_body.curvature;
        nearfield_world_buf->points.push_back(p_world);

        if (os_deskew_enable) {
            PendingNearfieldPoint pend;
            pend.t = t_nf;
            pend.p_body = Eigen::Vector3d(p_body.x, p_body.y, p_body.z);
            pend.intensity = p_body.intensity;
            pend.range = pend.p_body.norm();
            os_deskew_pending.push_back(pend);
        }
...
sensor_msgs::msg::PointCloud2 buildNearfieldMsg(const PointCloudXYZI::Ptr &pts,
    ...
    mod.resize(pts->points.size());
    sensor_msgs::PointCloud2Iterator<float> it_x(msg, "x"), it_y(msg, "y"), it_z(msg, "z"),
            it_t(msg, "time"), it_r(msg, "range"), it_i(msg, "intensity");
    for (size_t i = 0; i < pts->points.size(); ++i, ++it_x, ++it_y, ++it_z, ++it_t, ++it_r, ++it_i) {
        *it_x = pts->points[i].x;
        *it_y = pts->points[i].y;
        *it_z = pts->points[i].z;
        *it_t = pts->points[i].curvature / 1000.0f; // ms -> s, frame-relative offset time
        *it_r = ranges[i];
        *it_i = pts->points[i].intensity;
    }
    return msg;
}

// Finalize + publish this frame's near-field outputs. Must be called once per frame,
// after all flush_nearfield_upto() calls for that frame are done (i.e. after the k-loop).
void publish_nearfield(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubBody,
                       const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubWorld,
                       const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubRaw) {
    if (!p_pre->nearfield_enable || odom_only) {
        nearfield_raw_buf->clear();
        nearfield_world_buf->clear();
        return;
    }

    const size_t n = nearfield_world_buf->points.size();
    std::vector<float> ranges(n);
    for (size_t i = 0; i < n; i++) {
        const auto &p = nearfield_raw_buf->points[i];
        ranges[i] = std::sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
    }

    const rclcpp::Time stamp = get_ros_time(lidar_end_time);

    if (pubRaw && nearfield_publish_raw) {
        pubRaw->publish(buildNearfieldMsg(nearfield_raw_buf, ranges, stamp, "body"));
    }
    if (pubWorld && nearfield_publish_world) {
        pubWorld->publish(buildNearfieldMsg(nearfield_world_buf, ranges, stamp, odom_header_frame_id));
    }
    if (pubBody) {
        // Deskewed body frame @ frame-end: reproject the deskewed world points back through
        // the FINAL (frame-end) state, so every point in the output shares one common
        // reference pose/time -- this is what makes it "deskewed" rather than just "raw".
        V3D final_pos;
        Eigen::Quaterniond final_rot;
        if (!use_imu_as_input) {
            final_pos = kf_output.x_.pos;
            final_rot = kf_output.x_.rot.normalized();
        } else {
            final_pos = kf_input.x_.pos;
            final_rot = kf_input.x_.rot.normalized();
        }
        PointCloudXYZI::Ptr deskewed_body(new PointCloudXYZI());
        deskewed_body->points.resize(n);
        for (size_t i = 0; i < n; i++) {
            V3D p_w(nearfield_world_buf->points[i].x, nearfield_world_buf->points[i].y,
                   nearfield_world_buf->points[i].z);
            V3D p_b = final_rot.inverse() * (p_w - final_pos);
            deskewed_body->points[i].x = p_b(0);
            deskewed_body->points[i].y = p_b(1);
            deskewed_body->points[i].z = p_b(2);
            deskewed_body->points[i].intensity = nearfield_world_buf->points[i].intensity;
            deskewed_body->points[i].curvature = nearfield_world_buf->points[i].curvature;
        }
        pubBody->publish(buildNearfieldMsg(deskewed_body, ranges, stamp, "body"));
    }
...
// OS-Deskew C2: finalize and publish any pending near-field points whose fixed-lag
// window [t, t+lag] is now fully covered by the smoother's node history, transforming
// them through the RTS-smoothed pose instead of the causal one. Call once per frame,
// after flush_nearfield_upto() has populated os_deskew_pending for this frame. Output
// batches are NOT one-per-frame (points finalize whenever their own lag elapses, which
// straddles frame boundaries), so unlike /nearfield/*'s "time" field, these topics'
// per-point time is not populated (left at 0) -- there is no single frame to be relative to.
void publish_refined_if_ready(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubBody,
                              const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubWorld) {
    if (!os_deskew_enable || os_deskew_pending.empty()) return;

    const double t_ready = pose_smoother.latest_time() - os_deskew_lag;
    if (os_deskew_pending.front().t > t_ready) return;  // nothing finalizable yet

    std::vector<PendingNearfieldPoint> ready;
    while (!os_deskew_pending.empty() && os_deskew_pending.front().t <= t_ready) {
        ready.push_back(os_deskew_pending.front());
        os_deskew_pending.pop_front();
    }
    if (ready.empty()) return;

    pose_smoother.run_smoothing_pass();

    refined_world_buf->clear();
    refined_body_buf->clear();
    refined_ranges.clear();

    for (const auto &pt : ready) {
        Eigen::Matrix3d R_w, R_b;
        Eigen::Vector3d p_w, p_b;
        if (!pose_smoother.query(pt.t, R_w, p_w)) continue;                        // smoothed pose @ capture time
        if (!pose_smoother.query(pt.t + os_deskew_lag, R_b, p_b)) continue;        // smoothed pose @ +lag (body reference)

        const Eigen::Vector3d world_pt = R_w * pt.p_body + p_w;
        const Eigen::Vector3d body_pt = R_b.transpose() * (world_pt - p_b);

        PointType pw{};
        pw.x = world_pt(0); pw.y = world_pt(1); pw.z = world_pt(2);
        pw.intensity = pt.intensity; pw.curvature = 0;
        refined_world_buf->points.push_back(pw);

        PointType pb{};
        pb.x = body_pt(0); pb.y = body_pt(1); pb.z = body_pt(2);
        pb.intensity = pt.intensity; pb.curvature = 0;
        refined_body_buf->points.push_back(pb);

        refined_ranges.push_back(pt.range);
    }

    if (!refined_world_buf->points.empty()) {
        const rclcpp::Time stamp = get_ros_time(t_ready);
        if (pubWorld) pubWorld->publish(buildNearfieldMsg(refined_world_buf, refined_ranges, stamp, odom_header_frame_id));
        if (pubBody) pubBody->publish(buildNearfieldMsg(refined_body_buf, refined_ranges, stamp, "body"));
    }

    pose_smoother.trim_before(t_ready - os_deskew_retention_margin);
}

// OS-Deskew C2: feed the fixed-lag smoother's node stream, subsampled to os_deskew_node_hz
// (native EKF update rate is much higher). Called right after each time_seq group's EKF
// update succeeds, so `t` reflects a state that has actually been updated with all lidar
// data up to that point in time (not a mid-propagation state).
double last_smoother_feed_t = -1e18;
void feed_smoother_if_due(double t) {
    if (!os_deskew_enable) return;
    if (!pose_smoother_configured) {
        pose_smoother.configure(os_deskew_q_pos_accel, os_deskew_r_pos, os_deskew_r_vel,
                                os_deskew_q_rot, os_deskew_r_rot);
        pose_smoother_configured = true;
    }
    if (t - last_smoother_feed_t < 1.0 / os_deskew_node_hz) return;
    last_smoother_feed_t = t;

    if (!use_imu_as_input) {
        pose_smoother.add_node(t, kf_output.x_.rot.normalized().toRotationMatrix(),
                               kf_output.x_.pos, kf_output.x_.vel);
    } else {
        pose_smoother.add_node(t, kf_input.x_.rot.normalized().toRotationMatrix(),
                               kf_input.x_.pos, kf_input.x_.vel);
    }
}
...
int main(int argc, char **argv) {
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubNearfieldDeskewedBody;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubNearfieldDeskewedWorld;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubNearfieldRawBody;
...
//------------------------------------------------------------------------------------------------------
    ...
    while (rclcpp::ok()) {
        ...
        if (sync_packages(Measures)) {
            ...
            // reset the flush cursor; flushed group-by-group alongside feats_down_body below.
            sort(Measures.nearfield->points.begin(), Measures.nearfield->points.end(), time_list);
            nf_idx = 0;
            ...
            if (!use_imu_as_input) {
                ...
                for (k = 0; k < time_seq.size(); k++) {
                    ...
                    flush_nearfield_upto(time_current);
                    feed_smoother_if_due(time_current);
            ...
            // Safety-net flush for any near-field points past the last time_seq group
            // (e.g. float rounding at the very end of the frame); normally a no-op.
            flush_nearfield_upto(std::numeric_limits<double>::max());
            if (p_pre->nearfield_enable) {
                publish_nearfield(pubNearfieldDeskewedBody, pubNearfieldDeskewedWorld, pubNearfieldRawBody);
            }
            if (os_deskew_enable) {
                publish_refined_if_ready(pubNearfieldRefinedBody, pubNearfieldRefinedWorld);
            }
